...
# Read in the data
DualChoiceDataset <- read.csv("03_DualChoice_ObservedData.csv")

names(DualChoiceDataset) <- c("Original.Location", "Location", "Clock", "Date", "Treatment", "Repetition", "Treatment.Left", "Treatment.Right", "Responding", "Commment", "Video.Save.Time", "Determiantion.via.Video")

#Renaming Treatments & Adjusting data
DualChoiceDataset$Treatment <- gsub("Intortum", "D. intortum", DualChoiceDataset$Treatment)
DualChoiceDataset$Treatment <- gsub("Incanum", "D. incanum", DualChoiceDataset$Treatment)
DualChoiceDataset$Clock <- strptime(DualChoiceDataset$Clock, format = "%m/%d/%Y %H:%M:%S")
...
Filter_DF <- DualChoiceDataset %>% filter(Location != lag(Location, default = "x"))
...
labels <- c(
  Maize = "Maize",
  'D. intortum' = "<i>D.intortum</i>",
  'D. incanum' = "<i>D.incanum</i>"
)
...
plotAct2 <- ggplot(data = NLineWidth, aes(x = Segment, y = NSegChanges, group = Repetition, color = Settled, linetype = Settled)) +
  ...
  facet_wrap(~ Treatment, labeller = labeller(Treatment = labels)) +  # Apply custom facet labels
  ...
  theme(
    ...
    strip.text = element_markdown(size = 8.5),  # Render facet labels with markdown (allow HTML)
    text = element_text(size = 9),
    plot.margin = margin(0.2, 0.5, 1, 0.8, "cm"),
...
plotTreat <- agg_Location %>%
  ...
  theme_bw()+
  ylim(0,6.5)+
  facet_wrap(~Treatment, scale="free_x", labeller = labeller(Treatment = labels))+
  theme(
    text = element_text(size = 9),
    legend.text = element_text(size = 8),
    strip.text = element_markdown(size = 8.5),  # Enable Markdown for facets to support HTML
    plot.margin = margin(0.2, 0.5, 0.5, 0.8, "cm"),
    legend.position = c(0.5, -0.275),
    legend.direction = "horizontal",
    legend.background = element_rect(fill = "transparent"))+
